...
ptm <- proc.time()
foreach (row=1:nrow(firedata)) %dopar% {
  print(row)
  #TMAX
  ...
  for(i in -21:35){
    ...
    firedata[row,][[varnameTMAX]] <- tmaxDF[which(tmaxDF$Index == i),]$TMAX
  }
  #TMIN
  ...
  for(i in -21:35){
    ...
    firedata[row,][[varnameTMIN]] <- tminDF[which(tminDF$Index == i),]$TMIN
  }
  #TMPRCP
  ...
  for(i in -21:35){
    ...
    firedata[row,][[varnamePRCP]] <- prcpDF[which(prcpDF$Index == i),]$PRCP
  }
}
proc.time()-ptm
